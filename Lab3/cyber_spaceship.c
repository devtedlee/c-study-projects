...
const char* get_longest_safe_zone_or_null(const char* const cab_start_location, const size_t cab_length, const char* const cluster_start_locations[], const size_t cluster_lengths[], const size_t cluster_count, size_t* out_longest_length)
{
    ...
    size_t temp_safe_zone_length = 0;
    size_t layer = 0;
    ...
    for (temp_start_address = start_address; temp_start_address < end_address + 1; ++temp_start_address) {
        ...
        if (layer % 2 == 0) {
            if (temp_safe_zone_length == 0) {
                temp_safe_zone_start_address = temp_start_address;
            }
            ++temp_safe_zone_length;
        } else {
            temp_safe_zone_length = 0;
        }

        if (temp_safe_zone_length >= longest_length) {
            longest_length = temp_safe_zone_length;
            longest_cluster_start_address = temp_safe_zone_start_address;
        }
